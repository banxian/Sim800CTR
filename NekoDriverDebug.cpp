...
nn::fs::FileOutputStream* logfile = NULL;
...
{
    if (logfile == NULL) {
        logfile = new nn::fs::FileOutputStream;
        logfile->TryInitialize(L"sdmc:/Sim800.txt", true); // ?!

        if (logfile == NULL) {
            return 0; // Fake!
        }
        s32 writed;
        logfile->TryWrite(&writed, "================================\n", sizeof("================================\n") - 1, true);
    }
    char addresstext[40] = "";
    char bytestext[10]   = "";
    //char fulltext[50]    = "";
    ...
    if (logpos > 102400 * 50 - 80) {
        s32 writed;
        logfile->TryWrite(&writed, logbuff, logpos, true);
        //fflush(logfile);
        logpos = 0;
    }
    char psbin[9];
