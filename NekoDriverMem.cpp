...
void MemReset ()
{
    ...
    hardlog("InitInternalAddress\n");
    // InitInternalAddress
    theNekoDriver->InitInternalAddrs();

    hardlog("CpuInitialize\n");
    // Initialize the cpu
...
{
    nn::fs::FileInputStream mariofile;
    nn::Result r = mariofile.TryInitialize(filename.c_str());
    ...
    while (true) {
        s32 readed;
        mariofile.TryRead(&readed, fNorBuffer + 0x8000 * page + 0x4000, 0x4000);
        mariofile.TryRead(&readed, fNorBuffer + 0x8000 * page, 0x4000);
        page++;
        if (readed == 0) {
            break;
        }
...
{
    nn::fs::FileInputStream romfile;
    nn::Result r = romfile.TryInitialize(filename.c_str());
    hardlog("LoadBROM: TryInitialize: 0x%08X\n", r.GetPrintableBits());
    //if (fBROMBuffer == NULL){
...
bool TNekoDriver::SaveFullNorFlash()
{
    ...
    while (page < 0x10) {
        s32 writed;
        norfile.TryWrite(&writed, fNorBuffer + 0x8000 * page + 0x4000, 0x4000, true);
        norfile.TryWrite(&writed, fNorBuffer + 0x8000 * page, 0x4000, true);
        page++;
    }
    norfile.Finalize();
#ifdef CARD2
